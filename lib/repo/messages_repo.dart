...
class MessagesRepo {
  getMessageThreads(String myNumber,{String filter="",offset}) async {
    ...
    try {
      ...
      final apiResponse = await BaseRequesterMethods.baseRequester.baseGetAPI(
        Endpoints.GET_MESSAGE_THREADS(apiId, number,filter,offset),
      );
  ...
  getThreadMessages(String threadId, String myNumber , String offset) async {
    String? apiId = SharedPreferencesMethod.getString(StorageKeys.API_ID);
    try {
      String number = myNumber;
      final apiResponse = await BaseRequesterMethods.baseRequester.baseGetAPI(
        Endpoints.GET_THREAD_MESSAGES(apiId, number, threadId,offset),
      );
      if (!apiResponse['errors']) {
  ...
  Future<Messages?> sendMessage(String myNumber, String body, String to, [String? image]) async {
    ...
    try {
      ...
      final resp = await BaseRequesterMethods.baseRequester
          ...
          print("HEHEHEHEHEHEHHEHEHEHEHEHHEHEHHEHEHHE");
      print(resp);
      ...
      print(e.toString());
      print("I Am here in message repo and the error is here");
      return null;
    }
  }
}
