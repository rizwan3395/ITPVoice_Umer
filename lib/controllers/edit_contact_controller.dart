...
class EditContactController extends GetxController {
  ...
  TextEditingController get firstNameController => TextEditingController(text: "${contact!.firstname!}");
  TextEditingController get lastNameController => TextEditingController(text:  "${contact!.lastname}");
...
Future<String> removeCountryCode(String phoneNumber) async {
  try {
    final PhoneNumber parsedNumber = await PhoneNumberUtil().parse(phoneNumber);
    return parsedNumber.nationalNumber; // Returns the local number without the country code
  } catch (e) {
  ...
  addNewContactField() async {
    String localNumber = await removeCountryCode(contact!.phone!);
    ...
    contactFieldsData.add({
      ...
      "code": "1",
      "controller": TextEditingController(text: localNumber),
    });
    update();
  }
  ...
  saveContact() async {
    ...
    if (isFormValid) {
      ...
      try {
        var res = await contactsRepo.updateContact(contact!.pk, firstNameController.text, numbers[0].label ?? '',
